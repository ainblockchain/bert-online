...
"""
An online version of run_squad.py.
"""
...
def build_squad_examples(paragraph):
  """Convert a SQuAD json file into a list of SquadExample."""
  def is_whitespace(c):
...
def model_fn_builder(bert_config, init_checkpoint, num_train_steps,
...
def input_fn_builder(features_records, seq_length, drop_remainder):
  ...
  def input_fn(params):
    ...
    d = tf.data.TFRecordDataset.from_tensor_slices(features_records)
...
def print_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  """Print final predictions and log-odds of null if needed."""
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    example_id = example.qas_id + " / " + example.question_text
  ...
  print(json.dumps(all_nbest_json, indent=4) + "\n")

  if FLAGS.version_2_with_negative:
    print(json.dumps(scores_diff_json, indent=4) + "\n")
...
class FeatureProcessor(object):
  ...
  def __init__(self, features_records):
    self.features_records = features_records
    self.num_features = 0
  ...
  def process_feature(self, feature):
    ...
    self.features_records.append(tf_example.SerializeToString())
...
def main(_):
  ...
  eval_examples = build_squad_examples(example)

  eval_features_records = []
  eval_writer = FeatureProcessor(features_records=eval_features_records)
  eval_features = []
  ...
  predict_input_fn = input_fn_builder(
      features_records=eval_features_records,
